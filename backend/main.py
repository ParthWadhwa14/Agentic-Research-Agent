...
def build_file_artifacts(paths: List[str]) -> List[dict]:
    artifacts = []
    for path in paths:
        file_path = Path(path)
...
def image_search_response(run_id: str, query: str) -> AgentResponse:
    ...
    artifacts = build_file_artifacts(final_paths)
    if artifacts:
        answer = (
...
async def build_agent_response(
    ...
    return AgentResponse(
        ...
        answer=answer,
        artifacts=build_artifacts(run_id, normalized_format) if create_artifact else [],
    )
